...
if __name__ == "__main__":
    ...
    with FakeTensorMode():
        big_tensor = torch.randn(100000, 88, device='meta')
        if gpu_id == 0:
        ...
        def get_flattened_tensors(t):
            flattened_tensors = set()

            def recurse(obj):
                if type(obj) is torch.Tensor:
                    flattened_tensors.add(obj)
                else:
                    if gpu_id == 0:
                        print(type(obj))
                    attrs, _ = type(obj).__tensor_flatten__(obj)
                    maybe_flattened_tensors = [getattr(obj, attr) for attr in attrs]
                    for x in maybe_flattened_tensors:
                        recurse(x)
            recurse(t)
            return flattened_tensors

        flattened_t = get_flattened_tensors(my_dtensor)

        print([(flat_t.shape, flat_t.device) for flat_t in flattened_t])
